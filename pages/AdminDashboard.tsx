...
import { 
  ...
  Save, Camera, Loader2, ImageIcon, Layout, Info, Phone, Facebook, Video
} from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  const handleMediaUpload = async (file: File, isGallery: boolean = false) => {
    // Se for vídeo, processamos de forma diferente ou apenas base64 simples
  ...
  const handleCreatePost = async () => {
    ...
    try {
      // 1. Capa
      const imageUrl = await handleMediaUpload(blogImage);
      // 2. Galeria (Fotos e Vídeos)
      const gallery = await Promise.all(blogGallery.map(f => handleMediaUpload(f, true)));
      ...
      console.error(err);
      alert('Erro ao publicar. Nota: Vídeos grandes podem falhar devido ao limite do banco de dados.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
      ...
      {/* MODAL NOVO POST COM SUPORTE MULTIMÉDIA */}
      {isEditorOpen && (
             ...
             <div className="flex-grow overflow-y-auto p-8 md:p-16 space-y-12 no-scrollbar">
                ...
                <div className="space-y-8 p-10 bg-slate-50 rounded-[3rem] border border-slate-200">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    <h4 className="text-xl font-black text-slate-900 flex items-center gap-3"><Video className="text-emerald-500" /> Galeria Multimédia (Fotos e Vídeos)</h4>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {blogGallery.map((file, idx) => (
                        <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden shadow-md group">
                          ...
                          {file.type.startsWith('video/') && (
                             <div className="absolute bottom-2 left-2 bg-slate-900/80 p-1 px-2 rounded-md text-[8px] font-black text-white uppercase tracking-widest">Vídeo</div>
                          )}
                        </div>
                      ))}
                    </div>
